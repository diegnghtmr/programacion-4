public class ListaSimplementeEnlazada<T extends Comparable<T>> {
    ...
    public static <T extends Comparable<T>> ListaSimplementeEnlazada<T> concatenar(
            ListaSimplementeEnlazada<T> l1, ListaSimplementeEnlazada<T> l2) {

        ListaSimplementeEnlazada<T> lista = new ListaSimplementeEnlazada<>();

        Nodo<T> recorrerNodo = l1.getInicial();
        while(recorrerNodo != null){
            lista.agregarFinal(recorrerNodo.getDato());
            recorrerNodo = recorrerNodo.getSiguiente();
        }

        recorrerNodo = l2.getInicial();
        while (recorrerNodo != null) {
            lista.agregarFinal(recorrerNodo.getDato());
            recorrerNodo = recorrerNodo.getSiguiente();
        }
