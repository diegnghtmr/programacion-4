public class ListaSimplementeEnlazada<T extends Comparable<T>> {
    ...
    public ListaSimplementeEnlazada<T> obtenerPosicionesImpares(){
        ListaSimplementeEnlazada<T> lista = new ListaSimplementeEnlazada<>();
        if (inicial == null || inicial.getSiguiente() == null) return lista;
        Nodo<T> recorrerNodo = inicial;
        int index = 0;

        do {
            if ((index%2) != 0) lista.agregarFinal(recorrerNodo.getDato());
            recorrerNodo = recorrerNodo.getSiguiente();
            index++;
        } while(recorrerNodo != null);
        return lista;
